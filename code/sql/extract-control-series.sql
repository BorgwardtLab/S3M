...
DROP MATERIALIZED VIEW IF EXISTS control_vitals CASCADE;
create materialized view control_vitals as
SELECT  pvt.icustay_id, pvt.subject_id -- removed , pvt.hadm_id,
,	pvt.chart_time , 0 as sepsis_target
-- Easier names
...
FROM  (
  ...
  and ce.charttime between fd.intime and fd.outtime -- si.si_starttime -- choose chartevents between icu-entry and sepsis onset, but not more than 3 days before onset and not earlier than icu entry.
  ...
  --  on fd.icustay_id = s3c.icustay_id
  --left join sofachange sfc
  --  on fd.icustay_id = sfc.icustay_id
  -- exclude rows marked as error
 where fd.sepsis_control = 1 
    and ce.itemid in
  (
  ---- HEART RATE
  --211, --"Heart Rate"
